...
class Genre :
    ...
    def delete(self, conn):
        ...
        self.nom = str(input("Entrer le genre que vous voulez supprimer : "))
...
class Album:
    ...
    def affichage(conn):
        ID=str(input("Entrez l'ID de l'album à afficher ou tapez 0 pour voir la liste des albums"))
        if ID=='0':
            try:
            ...
            try:
                sql = "SELECT * FROM Album WHERE id=%s;"%ID
                cur = conn.cursor()
                cur.execute(sql)
